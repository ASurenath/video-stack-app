...
import { deleteCategApi } from '../apiService/allApi';


function Deletecateg({ categ, setcategDeleteUpdate }) {
    const [show, setShow] = useState(false);
    const handleClose = () => setShow(false);
    const handleShow = () => setShow(true);
    const handleDelete = async (e) => {
        e.preventDefault()
        const result = await deleteCategApi(categ?.id)
        setcategDeleteUpdate(result.data)
        ...
        <>
            ...
            <Modal show={show} onHide={handleClose} className="" data-bs-theme="dark">
                ...
                <Modal.Body className="bg-dark" style={{ border: "solid rgba(255, 255, 255, 0.050) 1px" }}>
                    <h4>Are you sure that you want to delete {categ.title}?</h4>
                </Modal.Body>
